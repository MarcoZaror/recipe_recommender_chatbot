...
class RecipeBot:
    def __init__(self):
        ...
        self.query['vegan'], self.query['vegetarian'], self.query['gluten'], self.query['lactosa'] = 0, 0, 0, 0
        self.tokenizer = Tokenizer()

    def init_bot(self, path_rec: Path, path_rec_emb: Path, path_w_emb: Path, path_w2i: Path):
    ...
    def get_recipes(self, phrase: str, embedding_size: int = 100):
        ...
        for i in range(1, (len(self.recipes) + 1)):
            if i in matchings:
                sims.append(cosine_similarity(self.recipes_emb[i].reshape(1,-1), phrase_emb.reshape(1,-1))[0])
